...
public class SolrOccurrenceWriter {
  ...
  /**
   * Validate if the latitude and longitude values are not null and have correct values:
   * latitude:[-90.0,90.0] and longitude[-180.0,180.0].
   */
  ...
  public void delete(List<Occurrence> input) throws IOException, SolrServerException {
    solrClient.deleteById(input.stream().map(occurrence -> occurrence.getKey().toString()).collect(Collectors.toList()), commitWithinMs);
  }
  ...
  public static SolrInputDocument buildOccSolrDocument(Occurrence occurrence) {
    ...
    doc.setField(BASIS_OF_RECORD.getFieldName(), occurrence.getBasisOfRecord() == null ? null : occurrence.getBasisOfRecord().name());
    ...
    doc.setField(TYPE_STATUS.getFieldName(), occurrence.getTypeStatus() == null ? null : occurrence.getTypeStatus().name());
    doc.setField(RECORD_NUMBER.getFieldName(), occurrence.getVerbatimField(DwcTerm.recordNumber));
    doc.setField(COUNTRY.getFieldName(), occurrence.getCountry() == null ? null : occurrence.getCountry().getIso2LetterCode());
    ...
    doc.setField(EVENT_DATE.getFieldName(), occurrence.getEventDate() != null ? toDateQueryFormat(occurrence.getEventDate()) : null);
    ...
    doc.setField(NETWORK_KEY.getFieldName(), occurrence.getNetworkKeys() == null || occurrence.getNetworkKeys().isEmpty() ? null
        : occurrence.getNetworkKeys().stream().map(UUID::toString).collect(Collectors.toList()));
