...
/**
 * Translates a Solr spell check response into a GBIF spell check response. moved from
 * common-search, see:
...
public class SpellCheckResponseBuilder {
  ...
  public static SpellCheckResponse build(org.apache.solr.client.solrj.response.SpellCheckResponse solrSpellCheckResponse) {
    ...
    // If the response contains collations, are use to build the response since those are guarantee to
    // get results
   ...
   */
  private static Map<String, SpellCheckResponse.Suggestion> fromCollatedResults(
      org.apache.solr.client.solrj.response.SpellCheckResponse solrSpellCheckResponse) {
   ...
   */
  private static Map<String, SpellCheckResponse.Suggestion> fromSuggestionsResults(
      org.apache.solr.client.solrj.response.SpellCheckResponse solrSpellCheckResponse) {
    Map<String, SpellCheckResponse.Suggestion> suggestionMap = Maps.newHashMap();
