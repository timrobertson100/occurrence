...
public class OccurrenceWsClientIT extends BaseResourceTest {
  ...
  public void init() throws Exception {
    ...
    Injector clientInjector = Guice.createInjector(new OccurrenceWsClientModule(properties), new AbstractModule() {
      @Override
  ...
  public void testAnnosysXml() {
    ...
    WebResource webResource =
        client.resource(wsBaseUrl).path(OccurrencePaths.OCCURRENCE_PATH).path(OccurrenceResource.ANNOSYS_PATH).path("10");
  ...
  public void testAnnosysVerbatimXml() {
    Client client = Client.create();
    WebResource webResource = client.resource(wsBaseUrl).path(OccurrencePaths.OCCURRENCE_PATH).path(OccurrenceResource.ANNOSYS_PATH)
        .path("10").path(OccurrencePaths.VERBATIM_PATH);
