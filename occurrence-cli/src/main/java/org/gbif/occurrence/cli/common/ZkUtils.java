...
public class ZkUtils {
   ...
   * starts a {@link org.apache.curator.framework.CuratorFramework}. These settings are not validated
   * in this method so only call it when the object has been validated.
   *
  ...
  public static CuratorFramework newCuratorFramework(ZooKeeperConfiguration cfg) throws IOException {
    CuratorFramework curator = CuratorFrameworkFactory.builder().namespace(cfg.namespace)
        .retryPolicy(new ExponentialBackoffRetry(cfg.baseSleepTime, cfg.maxRetries)).connectString(cfg.connectionString).build();
    curator.start();
    return curator;
  }
}
