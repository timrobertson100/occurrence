...
/**
 * Class that can emits delete message for all gbifid linked to previous crawls. Previous crawls
 * gbifid are received by a SQL query executed on a JDBC connection (e.g. Hive JDBC).
 *
 * Note: this class will issue a Thread.sleep the value of config.deleteMessageBatchIntervalMs on
 * each config.deleteMessageBatchSize emitted.
 */
class PreviousCrawlsOccurrenceDeleter {
  ...
  private static final String REGISTRY_COMMENT =
      "Auto-deletion of stale records: %d occurrence(s) flagged for deletion. %d was identified as the last normal crawl attempt.";

  private PreviousCrawlsManagerConfiguration config;
  private MessagePublisher publisher;
  private DatasetService datasetService;

  // we use a smaller than (<) instead of not equal (<>) to avoid deleting records of a potential new
  // crawl
  // that started
  private static final String SQL_QUERY_GET_OTHER_CRAWL_ID = "SELECT gbifId, crawlId FROM " + " %s WHERE datasetKey = ? AND crawlId < ?";

  private static final Function<String, String> getSqlCommand = (tableName) -> String.format(SQL_QUERY_GET_OTHER_CRAWL_ID, tableName);
   ...
   * @param datasetService used to log comments to the registry of what was deleted (this is not
   *        essential and could be removed once proven stable)
   */
  @Inject
  PreviousCrawlsOccurrenceDeleter(PreviousCrawlsManagerConfiguration config, MessagePublisher publisher, DatasetService datasetService) {
  ...
  private void sendDeleteMessage(int occurrenceKey, int lastSeenInCrawlId, int lastSuccessfulCrawlId) throws IOException {
    this.publisher.send(new DeleteOccurrenceMessage(occurrenceKey, OccurrenceDeletionReason.NOT_SEEN_IN_LAST_CRAWL, lastSeenInCrawlId,
        lastSuccessfulCrawlId));
  }

  /**
   * Record a message in the registry to keep a trace of automatic deletion. Eventually we might want
   * to only record deletion when the numberOfMessageEmitted is > than a certain threshold.
   * 
   * @param datasetKey
  ...
  /**
   * Sends delete message for all occurrence records that are coming from a crawl before
   * lastSuccessfulCrawl.
   *
   * @param datasetKey
   * @param lastSuccessfulCrawl
