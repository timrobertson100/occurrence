...
public class VerbatimProcessorService extends AbstractIdleService {
  ...
  protected void startUp() throws Exception {
    ...
    listener.listen(cfg.primaryQueueName, cfg.msgPoolSize, new FragmentPersistedListener(inj.getInstance(VerbatimProcessor.class)));
    listeners.add(listener);

    listener = new MessageListener(cfg.messaging.getConnectionParameters());
    listener.listen(cfg.secondaryQueueName, cfg.msgPoolSize, new ParseFragmentListener(inj.getInstance(VerbatimProcessor.class)));
    listeners.add(listener);
  }
