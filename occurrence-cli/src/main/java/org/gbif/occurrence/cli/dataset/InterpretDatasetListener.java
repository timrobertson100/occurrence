...
public class InterpretDatasetListener extends AbstractMessageCallback<InterpretDatasetMessage> {
  ...
  public InterpretDatasetListener(OccurrenceKeyPersistenceService occurrenceKeyService, MessagePublisher messagePublisher) {
  ...
  public void handleMessage(InterpretDatasetMessage message) {
    ...
    for (Integer key : keys) {
      try {
        ...
        LOG.warn("Could not send InterpretVerbatimMessage for key [{}] while interpreting dataset [{}]", key, message.getDatasetUuid(), e);
      }
    }
  }
}
