  ...
  static class ValidateTest extends HiveSQL.Validate {
    ...
    public HiveSQL.Validate.Result apply(String sql) {
      ...
      if (context.hasParseIssue())
        return new Result(context.sql(), context.translatedQuery(), issues, Arrays.asList(COMPILATION_ERROR), "", context,
            issues.isEmpty());
  ...
  public static Collection<Object[]> inputs() {
    return Arrays.asList(new Object[][] {
        ...
        {"SELECT `gbifid`, `countrycode`, `datasetkey`, `license` from occurrence", true, false, 0,
            "gbifid,countrycode,datasetkey,license"},
        ...
        {"SELECT `gbifid`, `countrycode`, `datasetkey`, `license`, `month`, `year` FROM `occurrence` WHERE `month`=3 AND `year` = 2018",
            true, false, 0, "gbifid,countrycode,datasetkey,license,month,year"},
        {"SELECT COUNT(`datasetkey`), `countrycode` ,`datasetkey` ,`license` FROM `occurrence` GROUP BY `countrycode`, `license`, `datasetkey`",
            true, false, 0, "COUNT(`datasetkey`),countrycode,datasetkey,license"},
        {"SELECT COUNT(`datasetkey`), `countrycode` ,`datasetkey`, `license` FROM `occurrence` GROUP BY `countrycode`, `license`, `datasetkey` HAVING count(`datasetkey`) > 5",
            false, false, 1, "COUNT(`datasetkey`),countrycode,datasetkey,license"},
        {"SELECT key FROM (SELECT key FROM src ORDER BY key LIMIT 10) UNION SELECT key FROM (SELECT key FROM src1 ORDER BY key LIMIT 10)",
            false, true, 1, ""}});
  }
