...
public class RegistryBasedOccurrenceMutatorTest {
  ...
  /**
   * Build a mock HBase {@link Result} that simulates what would be extracted from the
   * occurrence-persistence module.
   *
  ...
  public void testRequiresUpdateDataset() {
    ...
    assertTrue(StringUtils.isNotBlank(OCC_MUTATOR.generateUpdateMessage(org, org, datasetBefore, datasetAfter).orElse("")));
  }
  ...
  public void testRequiresUpdateDatasetOrganization() {
    ...
    assertTrue(StringUtils.isNotBlank(OCC_MUTATOR.generateUpdateMessage(orgBefore, orgAfter, dataset, dataset).orElse("")));
  }
