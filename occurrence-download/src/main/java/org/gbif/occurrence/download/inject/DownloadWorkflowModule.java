...
/**
 * Private guice module that provides bindings the required Modules and dependencies. The following
 * class are exposed: - CuratorFramework: this class is exposed only to close the zookeeper
 * connections properly. - OccurrenceFileWriter: class that creates the occurrence data and
 * citations file.
 */
public final class DownloadWorkflowModule extends AbstractModule {
  ...
  protected void configure() {
    ...
    install(new SolrModule(SolrConfig.fromProperties(workflowConfiguration.getDownloadSettings(), PROPERTIES_SOLR_PREFIX)));
  ...
  CuratorFramework provideCuratorFramework(@Named(PROPERTIES_PREFIX + "zookeeper.namespace") String zookeeperNamespace,
    ...
    CuratorFramework curator = CuratorFrameworkFactory.builder().namespace(zookeeperNamespace)
        .retryPolicy(new ExponentialBackoffRetry(sleepTime, maxRetries)).connectString(zookeeperConnection).build();
    curator.start();
    return curator;
  }
  ...
  @Provides
  ExecutionContextExecutorService provideExecutionContextExecutorService(@Named(PROPERTIES_PREFIX + "job.max_threads") int maxThreads) {
  ...
  @Provides
  LockFactory provideLock(CuratorFramework curatorFramework, @Named(PROPERTIES_PREFIX + "max_global_threads") Integer maxGlobalThreads) {
    return new ZooKeeperLockFactory(curatorFramework, maxGlobalThreads, LOCKING_PATH);
  }

  /**
   * Binds a DownloadFilesAggregator according to the DownloadFormat set using the key
   * DOWNLOAD_FORMAT_KEY.
   */
