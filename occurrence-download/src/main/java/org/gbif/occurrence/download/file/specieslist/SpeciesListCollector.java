...
public class SpeciesListCollector {
  ...
  public void collect(Map<String, String> occurrenceRecord) {
    ...
    distinctSpeciesRecord.put(taxonKey, distinctSpeciesRecord.compute(taxonKey, (k, v) -> {
      ...
      if (v != null) {
        long count = Long.parseLong(v.get(GbifTerm.numOfOccurrences.simpleName()))
            + Long.parseLong(occurrenceRecord.getOrDefault(GbifTerm.numOfOccurrences.simpleName(), "1"));
        ...
        return new LinkedHashMap<>(DownloadTerms.SPECIES_LIST_DOWNLOAD_TERMS.stream().collect(LinkedHashMap::new, (m, val) -> {
