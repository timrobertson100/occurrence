...
/**
 * Oozie Java action that creates a DwcA from hive tables. This action is used for big DwcA
 * downloads.
 */
public class ArchiveDownloadAction {
  ...
  /**
   * Entry point for assembling the dwc archive. The thrown exception is the only way of telling Oozie
  ...
  public static void main(String[] args) throws IOException {
    ...
    DownloadJobConfiguration configuration = new DownloadJobConfiguration.Builder().withDownloadKey(downloadKey)
        .withDownloadTableName(downloadTableName).withFilter(query).withIsSmallDownload(isSmallDownload).withUser(username)
        .withSourceDir(workflowConfiguration.getHiveDBPath()).withDownloadFormat(workflowConfiguration.getDownloadFormat()).build();
