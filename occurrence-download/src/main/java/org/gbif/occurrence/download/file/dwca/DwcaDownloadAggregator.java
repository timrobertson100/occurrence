...
public class DwcaDownloadAggregator implements DownloadAggregator {
  ...
  private static void appendResult(Result result, OutputStream interpretedFileWriter, OutputStream verbatimFileWriter,
    ...
    DownloadFileUtils.appendAndDelete(result.getDownloadFileWork().getJobDataFileName() + TableSuffixes.INTERPRETED_SUFFIX,
    ...
    DownloadFileUtils.appendAndDelete(result.getDownloadFileWork().getJobDataFileName() + TableSuffixes.VERBATIM_SUFFIX,
        verbatimFileWriter);
    DownloadFileUtils.appendAndDelete(result.getDownloadFileWork().getJobDataFileName() + TableSuffixes.MULTIMEDIA_SUFFIX,
        multimediaFileWriter);
  }

  @Inject
  public DwcaDownloadAggregator(OccurrenceDownloadService occurrenceDownloadService, DownloadJobConfiguration configuration) {
    ...
    try (FileOutputStream interpretedFileWriter = new FileOutputStream(configuration.getInterpretedDataFileName(), true);
      ...
      if (!results.isEmpty()) {
        ...
        CitationsFileWriter.createCitationFile(datasetUsagesCollector.getDatasetUsages(), configuration.getCitationDataFileName(),
            occurrenceDownloadService, configuration.getDownloadKey());
      }
