...
public class SimpleCsvArchiveBuilder {
  ...
  /**
   * Creates the file HEADER. It was moved to a function because a bug in javac
   * https://bugs.openjdk.java.net/browse/JDK-8077605.
   */
  public static SimpleCsvArchiveBuilder withHeader(Set<Term> downloadTermsHeader) {
    String header =
        downloadTermsHeader.stream().map(term -> term.simpleName().replaceAll("_", "")).collect(Collectors.joining("\t")) + '\n';
   ...
   * downloadKey.zip. The HEADER file is added to the directory hiveTableInputPath so it appears in
   * the resulting zip file.
   */
  public void mergeToZip(final FileSystem sourceFS, FileSystem targetFS, String sourcePath, String targetPath, String downloadKey,
      ModalZipOutputStream.MODE mode) throws IOException {
   ...
   */
  private void zipDefault(final FileSystem sourceFS, final FileSystem targetFS, String sourcePath, Path outputPath, String downloadKey) {
    try (FSDataOutputStream zipped = targetFS.create(outputPath, true); ZipOutputStream zos = new ZipOutputStream(zipped);) {
  ...
  private void zipPreDeflated(final FileSystem sourceFS, FileSystem targetFS, String sourcePath, Path outputPath, String downloadKey)
      throws IOException {
    try (FSDataOutputStream zipped = targetFS.create(outputPath, true);
      ...
      try {
        D2CombineInputStream in = new D2CombineInputStream(Arrays.stream(sourceFS.listStatus(inputPath)).map(fileStatus -> {
          try {
   ...
   */
  private void appendHeaderFile(FileSystem fileSystem, Path dir, ModalZipOutputStream.MODE mode) throws IOException {
  ...
  /**
   * Executes the archive/zip creation process. The expected parameters are: 0. sourcePath: HDFS path
   * to the directory that contains the data files. 1. targetPath: HDFS path where the resulting file
   * will be copied. 2. downloadKey: occurrence download key. 3. MODE: ModalZipOutputStream.MODE of
   * input files. 4. download-format : Download format 5. sql_header: tab separated SQL header in case
   * of SQL Download
   */
  public static void main(String[] args) throws IOException {
    ...
    if (DownloadFormat.valueOf(downloadFormat).equals(DownloadFormat.SQL)) {
      SimpleCsvArchiveBuilder.withHeader(args[5]).mergeToZip(sourceFileSystem, sourceFileSystem, args[0], args[1], args[2],
          ModalZipOutputStream.MODE.valueOf(args[3]));
    } else {
      Set<Term> downloadTerms =
          DownloadFormat.valueOf(downloadFormat).equals(DownloadFormat.SPECIES_LIST) ? DownloadTerms.SPECIES_LIST_DOWNLOAD_TERMS
              : DownloadTerms.SIMPLE_DOWNLOAD_TERMS;
      SimpleCsvArchiveBuilder.withHeader(downloadTerms).mergeToZip(sourceFileSystem, sourceFileSystem, args[0], args[1], args[2],
          ModalZipOutputStream.MODE.valueOf(args[3]));
    }
  }
