...
AS SELECT * FROM ${occurrenceTable}_es;

-- creates the citations table, citation table is not compressed since it is read later from Java as TSV.
-- country should be replaced by datasetkey in future, as dataset key is not a part of table
SET mapred.output.compress=false;
SET hive.exec.compress.output=false;
CREATE TABLE ${occurrenceTable}_citation ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' AS SELECT country, count(*) as num_occurrences FROM ${occurrenceTable} WHERE country IS NOT NULL GROUP BY country;
