...
AS SELECT COALESCE(acceptedtaxonkey, taxonkey) AS taxonkey, COALESCE(acceptedscientificname, scientificname) AS scientificname,
          acceptedtaxonkey, acceptedscientificname, taxonrank, taxonomicstatus, kingdom, kingdomkey, phylum, phylumkey, class,classkey,
          order_, orderkey, family, familykey, genus,genuskey, species, specieskey, datasetkey, license
...
AS SELECT taxonkey, scientificname, COUNT(taxonkey) AS numberOfOccurrences, acceptedtaxonkey, acceptedscientificname, taxonrank rank, taxonomicstatus, kingdom, kingdomkey,
