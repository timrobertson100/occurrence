...
public class TestDownloadHeaders {
  ...
  public void testTermsConsistency() {
    ...
    assertEquals("TermUtils.interpretedTerms() and Terms.interpretedTerms() must use the same terms. Difference(s): " + diff, 0,
        diff.size());
    ...
    assertEquals(
        "TermUtils.interpretedTerms() and DownloadTerms.DOWNLOAD_INTERPRETED_TERMS_HDFS must use the same terms. Difference(s): " + diff, 0,
        diff.size());
