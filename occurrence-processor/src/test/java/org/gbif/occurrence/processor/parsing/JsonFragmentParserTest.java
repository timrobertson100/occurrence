...
public class JsonFragmentParserTest {
  ...
  public void testParsePreferred() throws IOException {
    ...
    Fragment fragment = new Fragment(datasetKey, json.getBytes("UTF-8"), DigestUtils.md5(json.getBytes("UTF-8")),
        Fragment.FragmentType.JSON, EndpointType.DWC_ARCHIVE, new Date(), 1, OccurrenceSchemaType.DWCA, null, null);
    ...
    assertEquals("http://digit.snm.ku.dk/www/Aves/full/AVES-100348_Caprimulgus_pectoralis_fervidus_ad____f.jpg",
        verb.getExtensions().get(Extension.IMAGE).get(0).get(DcTerm.identifier));
  }
  ...
  public void testExtensions() throws IOException {
    ...
    // test extensions
    // TODO: The EOL extension is not recognized as the dwc-api dependency is outdated and does not
    // contain EOL terms...
    ...
    // assertEquals("http://www.mnh.si.edu/rc/db/2data_access_policy.html", m.get(DcTerm.license));
    // assertEquals("USNMENT832289", m.get(DcTerm.description));
    // assertEquals("National Museum of Natural History, Smithsonian Institution",
    // m.get(DcTerm.publisher));
    // assertEquals("http://collections.mnh.si.edu/media/index.php?irn=10842031c",
    // m.get(DcTerm.identifier));
    // assertEquals("Trichardis picta", m.get(DcTerm.title));
    // assertEquals("Image", m.get(DcTerm.type));
    // assertEquals("National Museum of Natural History, Smithsonian Institution",
    // m.get(DcTerm.rightsHolder));
    // assertEquals("general public", m.get(DcTerm.audience));
