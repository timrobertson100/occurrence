...
/**
 * Responsible for producing and persisting Fragment objects given crawling response snippets, and
 * sending a message reporting successful persistence. Also updates ZooKeeper with running counts of
 * success/failure.
 */
@Singleton
public class FragmentProcessor {
  ...
  // Previously been 3 times @ 500msecs, but this reduces crawling to 15/sec as there are 1.5 sec wait
  // times.
  ...
  private final Meter fragmentsProcessed = Metrics.newMeter(FragmentProcessor.class, "frags", "frags", TimeUnit.SECONDS);
  ...
  private final Timer findTimer = Metrics.newTimer(FragmentProcessor.class, "frag find time", TimeUnit.MILLISECONDS, TimeUnit.SECONDS);
  ...
  private final Timer msgTimer = Metrics.newTimer(FragmentProcessor.class, "msg send time", TimeUnit.MILLISECONDS, TimeUnit.SECONDS);
  ...
  @Inject
  public FragmentProcessor(FragmentPersistenceService fragmentPersister, OccurrenceKeyPersistenceService occurrenceKeyPersister,
  ...
  /**
   * Takes the given response/dwca fragment and parses it into Fragment object(s) and then persists
   * it/them. Responsible for identifying duplicates of existing Fragments and assigning ids to new
   * Fragments. The only case where multiple Fragments are returned from this message is ABCD 2.06
   * snippets that are differentiated with UnitQualifiers.
   *
   ...
   * @param validationReport the DwcaValidationReport if this fragment came from a DwC-A, null
   *        otherwise
   */
  public void buildFragments(UUID datasetKey, byte[] data, OccurrenceSchemaType schema, EndpointType protocol, Integer crawlId,
      @Nullable DwcaValidationReport validationReport) {
    ...
    LOG.debug("For dataset [{}] of type [{}] useTriplet is [{}] and useOccurrenceId is [{}]", datasetKey, schema,
        idStrategy.isTripletsValid(), idStrategy.isOccurrenceIdsValid());
    ...
    if (isXml) {
      ...
      try {
        extractionResults =
            XmlFragmentParser.extractIdentifiers(datasetKey, data, schema, idStrategy.isTripletsValid(), idStrategy.isOccurrenceIdsValid());
      } finally {
        context.stop();
      }
    } else {
      ...
      try {
        ...
        IdentifierExtractionResult result =
            JsonFragmentParser.extractIdentifiers(datasetKey, data, idStrategy.isTripletsValid(), idStrategy.isOccurrenceIdsValid());
        if (result != null) {
    ...
    if (extractionResults.isEmpty()) {
      LOG.debug(
          "Given snippet for dataset [{}] does not contain enough information to uniquely identify it - can not parse or " + "persist it",
          datasetKey.toString());
  ...
  /**
   * Process a single record into a single Fragment and persist it, generating a new key for it if
   * necessary.
   */
  private void processSingleFragment(Set<UniqueIdentifier> uniqueIds, boolean isXml, byte[] data, OccurrenceSchemaType schema,
      EndpointType protocol, Integer crawlId, String unitQualifier) {
    ...
    if (keyResult == null) {
      ...
      do {
        ...
        if (fragment == null) {
          /*
           * this means either 1) the key we're trying was only just assigned and the owner hasn't written
           * their fragment yet, or 2) our index is out of sync - it has an index entry pointing to nothing,
           * so delete the index entry(ies) and proceed as if this is an insert So we retry a few times to get
           * clear of 1), and then act on 2). Note however in POR-2807, that if for some reason deletions are
           * not clearing lookups (i.e. POR-995) too conservative rates mean the system grinds to a complete
           * halt for large datasets.
           */
    ...
    try {
      if (status == OccurrencePersistenceStatus.NEW) {
        ...
        if (!creationResult.isKeyCreated()) {
          ...
          status = OccurrencePersistenceStatus.UPDATED;
          LOG.info("Fragment creation did not generate new key - lost race and now using existing [{}] as status [UPDATE]",
              fragment.getKey());
        }
      } else {
  ...
  private void failForDataStateException(UUID datasetKey, Throwable e) {
    LOG.warn("Data inconsistency prevents this fragment being processed for dataset [{}]: {}", datasetKey, e.getMessage(), e);
    updateZookeeper(datasetKey, ZookeeperConnector.CounterName.RAW_OCCURRENCE_PERSISTED_ERROR);
  }
