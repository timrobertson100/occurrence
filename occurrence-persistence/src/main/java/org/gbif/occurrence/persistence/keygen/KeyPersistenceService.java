...
/**
 * Implementations of this interface will be responsible for finding, generating and deleting the
 * secondary index keys that are used to lookup entries by their unique identifiers. The initial
 * target implementation is for occurrence records looked up by their
 * {@link org.gbif.occurrence.common.identifier.UniqueIdentifier}. The type parameter refers to the
 * type of the entry key in the primary index (e.g. for occurrences this is an integer). It's
...
public interface KeyPersistenceService<T> {
  ...
  /**
   * Given the strings that are unique within the given scope, find the key for the single record that
   * matches all of the given strings.
   *
  ...
  /**
   * Generates a new key for the given unique strings and their scope. If an existing key is found for
   * these strings that is returned instead. The {@link KeyLookupResult} will indicate whether the key
  ...
  /**
   * Delete the entries in the secondary index corresponding to the passed in uniqueStrings and scope
   * (e.g. delete occurrence lookup by triplet and dataset).
   *
   * @param uniqueStrings strings unique within the scope
   * @param scope a scope for the strings' uniqueness (e.g. a dataset key/name)
   */
