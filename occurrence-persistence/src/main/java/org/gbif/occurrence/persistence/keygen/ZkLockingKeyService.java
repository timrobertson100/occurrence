...
/**
 * An extension of AbstracHBaseKeyPersistenceService with a generateKey implementation that uses a
 * thread local ZookeeperLockManager to ensure thread-safe key gen.
 *
 * NOTE: this class is currently unused, as it becomes very slow when used concurrently by many
 * threads.
 */
public class ZkLockingKeyService extends AbstractHBaseKeyPersistenceService {
  ...
  @Inject
  public ZkLockingKeyService(OccHBaseConfiguration cfg, Connection connection, ThreadLocalLockProvider zookeeperLockManagerProvider) {
    super(cfg, connection, new OccurrenceKeyBuilder());
    this.zookeeperLockManagerProvider = zookeeperLockManagerProvider;
  }

  /**
   * Takes out a lock in zookeeper for the dataset of this occurrence before generating a key for it.
   * It first checks to see if a key already exists for this set of uniqueIdentifiers before
  ...
  public KeyLookupResult generateKey(Set<String> uniqueStrings, String scope) {
    ...
    // check again if it already exists, although now we expect it to be null - we have to have the lock
    // for this
    // because if we check for existence before getting the lock then someone could write a new key and
    // drop the lock
    // between us checking (finding nothing) and getting the lock and then writing a new (and now
    // incorrect) key
    try {
      ...
      Long longKey = counterTableStore.incrementColumnValue(HBaseLockingKeyService.COUNTER_ROW, Columns.COUNTER_COLUMN, 1);
      if (longKey > Integer.MAX_VALUE) {
        throw new IllegalStateException("The next available occurrence id is greater than what Integer can handle. This is fatal!");
      }
