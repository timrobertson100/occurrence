...
/**
 * This service manages the secondary index that links unique identifiers (either holy triplet or
 * publisher-provided unique ids) to GBIF-generated occurrence keys (integers). It is also
 * responsible for the generation of those GBIF occurrence keys.
 *
 * TODO: the KeyPersistenceService class is very similar to this and adds confusion - they should be
 * combined
 */
public interface OccurrenceKeyPersistenceService {

  /**
   * Given a set of UniqueIdentifiers, fetch the GBIF-generated occurrence key for this occurrence
   * from the occurrence key lookup table, if it exists.
   *
  ...
  /**
   * If a key already exists for these uniqueIdentifiers, returns the existing key, otherwise
   * generates a new key.
   *
   ...
   * @param datasetKey without the datasetKey the deletion will do a full table scan of lookups (very
   *        slow)
   *
   * @throws org.gbif.api.exception.ServiceUnavailableException if the underlying data connection
   *         fails
